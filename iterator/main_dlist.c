...
static void __dlist_iterator_next(iterator_t *p_iter) //让迭代器指向容器的下一个数据
{
	*p_iter = ((dlist_node_t *)*p_iter)->p_next;
}

static void __dlist_iterator_prev(iterator_t *p_iter) //让迭代器指向容器的上一个数据
{
...
int main(void)
{
	...
	iter_foreach(&iterator_if,dlist_begin_get(&head), dlist_end_get(&head), __visit, NULL);
#else
